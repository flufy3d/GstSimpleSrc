/**
 * SECTION:element-simplesrc
 * @title: simplesrc
 *
 * The simplesrc element is used to produce test video data in a wide variety
 * of formats. The video test data produced can be controlled with the "pattern"
 * property.
 *
 * By default the simplesrc will generate data indefinitely, but if the
 * #GstBaseSrc:num-buffers property is non-zero it will instead generate a
 * fixed number of video frames and then send EOS.
 *
 * ## Example launch line
 * |[
 * gst-launch-1.0 -v simplesrc pattern=snow ! video/x-raw,width=1280,height=720 ! autovideosink
 * ]|
...
GST_DEBUG_CATEGORY_STATIC (simple_src_debug);
#define GST_CAT_DEFAULT simple_src_debug

#define DEFAULT_PATTERN            GST_simple_SRC_SMPTE
#define DEFAULT_ANIMATION_MODE     GST_simple_SRC_FRAMES
#define DEFAULT_MOTION_TYPE        GST_simple_SRC_WAVY
#define DEFAULT_FLIP               FALSE
#define DEFAULT_TIMESTAMP_OFFSET   0
#define DEFAULT_IS_LIVE            FALSE
#define DEFAULT_COLOR_SPEC         GST_simple_SRC_BT601
...
static GstStaticPadTemplate gst_simple_src_template =
...
#define gst_simple_src_parent_class parent_class
G_DEFINE_TYPE (GSTSIMPLESRC, gst_simple_src, GST_TYPE_PUSH_SRC);

static void gst_simple_src_set_pattern (GSTSIMPLESRC * simplesrc,
    int pattern_type);
static void gst_simple_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_simple_src_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_simple_src_setcaps (GstBaseSrc * bsrc, GstCaps * caps);
static GstCaps *gst_simple_src_src_fixate (GstBaseSrc * bsrc,
    GstCaps * caps);

static gboolean gst_simple_src_is_seekable (GstBaseSrc * psrc);
static gboolean gst_simple_src_do_seek (GstBaseSrc * bsrc,
    GstSegment * segment);
static gboolean gst_simple_src_query (GstBaseSrc * bsrc, GstQuery * query);

static void gst_simple_src_get_times (GstBaseSrc * basesrc,
    GstBuffer * buffer, GstClockTime * start, GstClockTime * end);
static gboolean gst_simple_src_decide_allocation (GstBaseSrc * bsrc,
    GstQuery * query);
static GstFlowReturn gst_simple_src_fill (GstPushSrc * psrc,
    GstBuffer * buffer);
static gboolean gst_simple_src_start (GstBaseSrc * basesrc);
static gboolean gst_simple_src_stop (GstBaseSrc * basesrc);

#define GST_TYPE_simple_SRC_PATTERN (gst_simple_src_pattern_get_type ())
static GType
gst_simple_src_pattern_get_type (void)
{
  static GType simple_src_pattern_type = 0;
  static const GEnumValue pattern_types[] = {
    {GST_simple_SRC_SMPTE, "SMPTE 100% color bars", "smpte"},
    {GST_simple_SRC_SNOW, "Random (television snow)", "snow"},
    {GST_simple_SRC_BLACK, "100% Black", "black"},
    {GST_simple_SRC_WHITE, "100% White", "white"},
    {GST_simple_SRC_RED, "Red", "red"},
    {GST_simple_SRC_GREEN, "Green", "green"},
    {GST_simple_SRC_BLUE, "Blue", "blue"},
    {GST_simple_SRC_CHECKERS1, "Checkers 1px", "checkers-1"},
    {GST_simple_SRC_CHECKERS2, "Checkers 2px", "checkers-2"},
    {GST_simple_SRC_CHECKERS4, "Checkers 4px", "checkers-4"},
    {GST_simple_SRC_CHECKERS8, "Checkers 8px", "checkers-8"},
    {GST_simple_SRC_CIRCULAR, "Circular", "circular"},
    {GST_simple_SRC_BLINK, "Blink", "blink"},
    {GST_simple_SRC_SMPTE75, "SMPTE 75% color bars", "smpte75"},
    {GST_simple_SRC_ZONE_PLATE, "Zone plate", "zone-plate"},
    {GST_simple_SRC_GAMUT, "Gamut checkers", "gamut"},
    {GST_simple_SRC_CHROMA_ZONE_PLATE, "Chroma zone plate",
        "chroma-zone-plate"},
    {GST_simple_SRC_SOLID, "Solid color", "solid-color"},
    {GST_simple_SRC_BALL, "Moving ball", "ball"},
    {GST_simple_SRC_SMPTE100, "SMPTE 100% color bars", "smpte100"},
    {GST_simple_SRC_BAR, "Bar", "bar"},
    {GST_simple_SRC_PINWHEEL, "Pinwheel", "pinwheel"},
    {GST_simple_SRC_SPOKES, "Spokes", "spokes"},
    {GST_simple_SRC_GRADIENT, "Gradient", "gradient"},
    {GST_simple_SRC_COLORS, "Colors", "colors"},
    {0, NULL, NULL}
  };

  if (!simple_src_pattern_type) {
    simple_src_pattern_type =
        g_enum_register_static ("GSTSIMPLESRCPattern", pattern_types);
  }
  return simple_src_pattern_type;
}
...
#define GST_TYPE_simple_SRC_ANIMATION_MODE (gst_simple_src_animation_mode_get_type ())
static GType
gst_simple_src_animation_mode_get_type (void)
{
  static GType simple_src_animation_mode = 0;
  static const GEnumValue animation_modes[] = {

    {GST_simple_SRC_FRAMES, "frame count", "frames"},
    {GST_simple_SRC_WALL_TIME, "wall clock time", "wall-time"},
    {GST_simple_SRC_RUNNING_TIME, "running time", "running-time"},
    {0, NULL, NULL}
  };

  if (!simple_src_animation_mode) {
    simple_src_animation_mode =
        g_enum_register_static ("GSTSIMPLESRCAnimationMode",
        animation_modes);
  }
  return simple_src_animation_mode;
}


#define GST_TYPE_simple_SRC_MOTION_TYPE (gst_simple_src_motion_type_get_type ())
static GType
gst_simple_src_motion_type_get_type (void)
{
  static GType simple_src_motion_type = 0;
  static const GEnumValue motion_types[] = {
    {GST_simple_SRC_WAVY, "Ball waves back and forth, up and down",
        "wavy"},
    {GST_simple_SRC_SWEEP, "1 revolution per second", "sweep"},
    {GST_simple_SRC_HSWEEP, "1/2 revolution per second, then reset to top",
        "hsweep"},
    {0, NULL, NULL}
  };

  if (!simple_src_motion_type) {
    simple_src_motion_type =
        g_enum_register_static ("GSTSIMPLESRCMotionType", motion_types);
  }
  return simple_src_motion_type;
}


static void
gst_simple_src_class_init (GSTSIMPLESRCClass * klass)
{
  ...
  gobject_class->set_property = gst_simple_src_set_property;
  gobject_class->get_property = gst_simple_src_get_property;

  g_object_class_install_property (gobject_class, PROP_PATTERN,
      g_param_spec_enum ("pattern", "Pattern",
          "Type of test pattern to generate", GST_TYPE_simple_SRC_PATTERN,
  ...
  g_object_class_install_property (gobject_class, PROP_ANIMATION_MODE,
      g_param_spec_enum ("animation-mode", "Animation mode",
          ...
          GST_TYPE_simple_SRC_ANIMATION_MODE, DEFAULT_ANIMATION_MODE,
  ...
  g_object_class_install_property (gobject_class, PROP_MOTION_TYPE,
      g_param_spec_enum ("motion", "Motion",
          ...
          GST_TYPE_simple_SRC_MOTION_TYPE, DEFAULT_MOTION_TYPE,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  ...
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_simple_src_template);

  gstbasesrc_class->set_caps = gst_simple_src_setcaps;
  gstbasesrc_class->fixate = gst_simple_src_src_fixate;
  gstbasesrc_class->is_seekable = gst_simple_src_is_seekable;
  gstbasesrc_class->do_seek = gst_simple_src_do_seek;
  gstbasesrc_class->query = gst_simple_src_query;
  gstbasesrc_class->get_times = gst_simple_src_get_times;
  gstbasesrc_class->start = gst_simple_src_start;
  gstbasesrc_class->stop = gst_simple_src_stop;
  gstbasesrc_class->decide_allocation = gst_simple_src_decide_allocation;

  gstpushsrc_class->fill = gst_simple_src_fill;
}

static void
gst_simple_src_init (GSTSIMPLESRC * src)
{
  gst_simple_src_set_pattern (src, DEFAULT_PATTERN);
...
static GstCaps *
gst_simple_src_src_fixate (GstBaseSrc * bsrc, GstCaps * caps)
{
  GSTSIMPLESRC *src = GST_simple_SRC (bsrc);
...
static void
gst_simple_src_set_pattern (GSTSIMPLESRC * simplesrc,
    int pattern_type)
{
  simplesrc->pattern_type = pattern_type;

  GST_DEBUG_OBJECT (simplesrc, "setting pattern to %d", pattern_type);

  switch (pattern_type) {
    case GST_simple_SRC_SMPTE:
      simplesrc->make_image = gst_simple_src_smpte;
      break;
    case GST_simple_SRC_SNOW:
      simplesrc->make_image = gst_simple_src_snow;
      break;
    case GST_simple_SRC_BLACK:
      simplesrc->make_image = gst_simple_src_black;
      break;
    case GST_simple_SRC_WHITE:
      simplesrc->make_image = gst_simple_src_white;
      break;
    case GST_simple_SRC_RED:
      simplesrc->make_image = gst_simple_src_red;
      break;
    case GST_simple_SRC_GREEN:
      simplesrc->make_image = gst_simple_src_green;
      break;
    case GST_simple_SRC_BLUE:
      simplesrc->make_image = gst_simple_src_blue;
      break;
    case GST_simple_SRC_CHECKERS1:
      simplesrc->make_image = gst_simple_src_checkers1;
      break;
    case GST_simple_SRC_CHECKERS2:
      simplesrc->make_image = gst_simple_src_checkers2;
      break;
    case GST_simple_SRC_CHECKERS4:
      simplesrc->make_image = gst_simple_src_checkers4;
      break;
    case GST_simple_SRC_CHECKERS8:
      simplesrc->make_image = gst_simple_src_checkers8;
      break;
    case GST_simple_SRC_CIRCULAR:
      simplesrc->make_image = gst_simple_src_circular;
      break;
    case GST_simple_SRC_BLINK:
      simplesrc->make_image = gst_simple_src_blink;
      break;
    case GST_simple_SRC_SMPTE75:
      simplesrc->make_image = gst_simple_src_smpte75;
      break;
    case GST_simple_SRC_ZONE_PLATE:
      simplesrc->make_image = gst_simple_src_zoneplate;
      break;
    case GST_simple_SRC_GAMUT:
      simplesrc->make_image = gst_simple_src_gamut;
      break;
    case GST_simple_SRC_CHROMA_ZONE_PLATE:
      simplesrc->make_image = gst_simple_src_chromazoneplate;
      break;
    case GST_simple_SRC_SOLID:
      simplesrc->make_image = gst_simple_src_solid;
      break;
    case GST_simple_SRC_BALL:
      simplesrc->make_image = gst_simple_src_ball;
      break;
    case GST_simple_SRC_SMPTE100:
      simplesrc->make_image = gst_simple_src_smpte100;
      break;
    case GST_simple_SRC_BAR:
      simplesrc->make_image = gst_simple_src_bar;
      break;
    case GST_simple_SRC_PINWHEEL:
      simplesrc->make_image = gst_simple_src_pinwheel;
      break;
    case GST_simple_SRC_SPOKES:
      simplesrc->make_image = gst_simple_src_spokes;
      break;
    case GST_simple_SRC_GRADIENT:
      simplesrc->make_image = gst_simple_src_gradient;
      break;
    case GST_simple_SRC_COLORS:
      simplesrc->make_image = gst_simple_src_colors;
      break;
    default:
      g_assert_not_reached ();
  }
...
static void
gst_simple_src_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GSTSIMPLESRC *src = GST_simple_SRC (object);

  switch (prop_id) {
    case PROP_PATTERN:
      gst_simple_src_set_pattern (src, g_value_get_enum (value));
      break;
...
static void
gst_simple_src_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GSTSIMPLESRC *src = GST_simple_SRC (object);
...
static gboolean
gst_simple_src_parse_caps (const GstCaps * caps,
...
no_framerate:
  {
    GST_DEBUG ("simplesrc no framerate given");
    return FALSE;
  }
invalid_format:
  {
    GST_DEBUG ("simplesrc invalid bayer format given");
    return FALSE;
  }
}

static gboolean
gst_simple_src_decide_allocation (GstBaseSrc * bsrc, GstQuery * query)
{
  GSTSIMPLESRC *simplesrc;
  GstBufferPool *pool;
  gboolean update;
  guint size, min, max;
  GstStructure *config;
  ...
  simplesrc = GST_simple_SRC (bsrc);

  if (gst_query_get_n_allocation_pools (query) > 0) {
    gst_query_parse_nth_allocation_pool (query, 0, &pool, &size, &min, &max);

    /* adjust size */
    size = MAX (size, simplesrc->info.size);
    update = TRUE;
  } else {
    pool = NULL;
    size = simplesrc->info.size;
    min = max = 0;
    update = FALSE;
  }
...
static gboolean
gst_simple_src_setcaps (GstBaseSrc * bsrc, GstCaps * caps)
{
  const GstStructure *structure;
  GSTSIMPLESRC *simplesrc;
  GstVideoInfo info;
  ...
  if (gst_structure_has_name (structure, "video/x-raw")) {
    ...
    if (!gst_simple_src_parse_caps (caps, &info.width, &info.height,
    ...
    simplesrc->bayer = TRUE;
    simplesrc->x_invert = x_inv;
    simplesrc->y_invert = y_inv;
  } else {
    goto unsupported_caps;
  }

  /* create chroma subsampler */
  if (simplesrc->subsample)
    gst_video_chroma_resample_free (simplesrc->subsample);
  simplesrc->subsample = gst_video_chroma_resample_new (0,
      info.chroma_site, 0, info.finfo->unpack_format, -info.finfo->w_sub[2],
      -info.finfo->h_sub[2]);

  for (i = 0; i < simplesrc->n_lines; i++)
    g_free (simplesrc->lines[i]);
  g_free (simplesrc->lines);

  if (simplesrc->subsample != NULL) {
    gst_video_chroma_resample_get_info (simplesrc->subsample,
        &n_lines, &offset);
  } else {
    n_lines = 1;
    offset = 0;
  ...
  simplesrc->lines = g_malloc (sizeof (gpointer) * n_lines);
  for (i = 0; i < n_lines; i++)
    simplesrc->lines[i] = g_malloc ((info.width + 16) * 8);
  simplesrc->n_lines = n_lines;
  simplesrc->offset = offset;

  /* looks ok here */
  simplesrc->info = info;

  GST_DEBUG_OBJECT (simplesrc, "size %dx%d, %d/%d fps",
      info.width, info.height, info.fps_n, info.fps_d);

  g_free (simplesrc->tmpline);
  g_free (simplesrc->tmpline2);
  g_free (simplesrc->tmpline_u8);
  g_free (simplesrc->tmpline_u16);
  simplesrc->tmpline_u8 = g_malloc (info.width + 8);
  simplesrc->tmpline = g_malloc ((info.width + 8) * 4);
  simplesrc->tmpline2 = g_malloc ((info.width + 8) * 4);
  simplesrc->tmpline_u16 = g_malloc ((info.width + 16) * 8);

  simplesrc->accum_rtime += simplesrc->running_time;
  simplesrc->accum_frames += simplesrc->n_frames;

  simplesrc->running_time = 0;
  simplesrc->n_frames = 0;

  GST_OBJECT_UNLOCK (simplesrc);
...
static gboolean
gst_simple_src_query (GstBaseSrc * bsrc, GstQuery * query)
{
  gboolean res = FALSE;
  GSTSIMPLESRC *src;

  src = GST_simple_SRC (bsrc);
...
static void
gst_simple_src_get_times (GstBaseSrc * basesrc, GstBuffer * buffer,
...
static gboolean
gst_simple_src_do_seek (GstBaseSrc * bsrc, GstSegment * segment)
{
...
static gboolean
gst_simple_src_is_seekable (GstBaseSrc * psrc)
{
  /* we're seekable... */
  return TRUE;
}
...
gst_simple_src_fill (GstPushSrc * psrc, GstBuffer * buffer)
{
  GSTSIMPLESRC *src;
...
static gboolean
gst_simple_src_start (GstBaseSrc * basesrc)
{
  GSTSIMPLESRC *src = GST_simple_SRC (basesrc);
...
static gboolean
gst_simple_src_stop (GstBaseSrc * basesrc)
{
  GSTSIMPLESRC *src = GST_simple_SRC (basesrc);
  guint i;
...
{
  GST_DEBUG_CATEGORY_INIT (simple_src_debug, "simplesrc", 0,
      "Video Test Source");

  return gst_element_register (plugin, "simplesrc", GST_RANK_NONE,
      GST_TYPE_simple_SRC);
}
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    simplesrc,
    "Creates a simple test video stream",
